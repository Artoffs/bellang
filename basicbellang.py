...
LETTERS = 'абвгдежзійклмнопрстуўфхцчшыьэюяи' + 'абвгдежзійклмнопрстуўфхцчшыьэюяи'.upper() + string.ascii_letters
...
class Error:
    ...
    def as_string(self):
        ...
        result += f'Файл {self.pos_start.fn}, лінія {self.pos_start.ln + 1}'
...
class IllegalCharError(Error):
    def __init__(self, pos_start, pos_end, details):
        super().__init__(pos_start, pos_end, 'Забаронены сімвал', details)


class ExpectedCharError(Error):
    def __init__(self, pos_start, pos_end, details=''):
        super().__init__(pos_start, pos_end, 'Чаканы сімвал', details)


class InvalidSyntaxError(Error):
    def __init__(self, pos_start, pos_end, details=''):
        super().__init__(pos_start, pos_end, 'Няправільны сінтаксіс', details)


class RTError(Error):
    def __init__(self, pos_start, pos_end, details, context):
        super().__init__(pos_start, pos_end, 'Памылка выканання', details)
    ...
    def generate_traceback(self):
        ...
        while ctx:
            result = f'  Файл {pos.fn}, лінія {str(pos.ln + 1)}, у {ctx.display_name}\n' + result
            pos = ctx.parent_entry_pos
            ctx = ctx.parent

        return 'Зваротная сувязь (самы апошні выклік):\n' + result
...
class Lexer:
    ...
    def make_not_equals(self):
        ...
        self.advance()
        return None, ExpectedCharError(pos_start, self.pos, "'=' (пасля '!')")
...
class Parser:
    ...
    def if_expr(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'кали'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Чакана 'кали'"
            ))
        ...
        while self.current_tok.matches(TT_KEYWORD, 'интады'):
            res.register_advancement()
            self.advance()
    ...
    def atom(self):
        ...
        elif tok.matches(TT_KEYWORD, 'пакуль'):
            while_expr = res.register(self.while_expr())
            if res.error:
                return res
            return res.success(while_expr)

        elif tok.matches(TT_KEYWORD, 'функцыя'):
...
global_symbol_table.set("Хлусня", Number.false)
global_symbol_table.set("Прауда", Number.true)
global_symbol_table.set("Пі", Number.Math_PI)
global_symbol_table.set("друкаваць", BuiltInFunction.print)
global_symbol_table.set("друкаваць_р", BuiltInFunction.print_ret)
global_symbol_table.set("увод", BuiltInFunction.input)
global_symbol_table.set("увод_лік", BuiltInFunction.input_int)
global_symbol_table.set("лік_лі", BuiltInFunction.is_number)
global_symbol_table.set("радок_лі", BuiltInFunction.is_string)
global_symbol_table.set("спіс_лі", BuiltInFunction.is_list)
global_symbol_table.set("фунуцыя_лі", BuiltInFunction.is_function)
global_symbol_table.set("дадаць", BuiltInFunction.append)
global_symbol_table.set("удаліць", BuiltInFunction.pop)
global_symbol_table.set("пашырыць", BuiltInFunction.extend)
