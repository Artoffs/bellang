...
class Parser:
    ...
    def atom(self):
        ...
        return res.failure(InvalidSyntaxError(
            ...
            "Чаканы цэлы лік, дробны, ідэнтыфікатар, '+', '-', або '('"
        ))
        ...
        if self.current_tok.type == TT_LPAREN:
            ...
            if self.current_tok.type == TT_RPAREN:
                ...
                if res.error:
                    return res.failure(InvalidSyntaxError(
                        self.current_tok.pos_start, self.current_tok.pos_end,
                        "Чакана ')', 'ПЕР', 'для', 'пакуль', 'функция', цэлы лик, дробны лик, идэнтэфикатар,"
                    ))
...
global_symbol_table.set("НУЛЬ", Number(0))
global_symbol_table.set("Хлусня", Number(0))
global_symbol_table.set("Прауда", Number(1))
